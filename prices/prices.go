...
func calculateTaxedPrice(price float64, taxRate float64) float64 {
	return helpers.Round(price*(100+taxRate)/100, 2)
}
...
func (job *TaxIncludedPriceJob) Process() {
	for _, price := range job.Prices {
		job.TaxedPrices[fmt.Sprintf("%.2f", price)] = calculateTaxedPrice(price, job.TaxRate)
	}
}
